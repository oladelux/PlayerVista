 ...
 * Create a new player
 */
export const createNewPlayerThunk = createAsyncThunk<unknown, { data: PlayerFormData }>(
  'players/addPlayer',
...
>('players/team', ({ teamId }) => {
  if (teamId) return getPlayersByTeamId(teamId)
})
...
export const getPlayerByIdThunk = createAsyncThunk<undefined | Player, { id: string }>(
  'players/player',
  async ({ id }) => {
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(createNewPlayerThunk.fulfilled, state => {
        if (state.loadingCreatingNewPlayerStatus === 'pending') {
      ...
      .addCase(updatePlayerThunk.fulfilled, state => {
        if (state.loadingUpdatingPlayer === 'pending') {
      ...
      .addCase(getPlayerByIdThunk.fulfilled, (state, action) => {
        if (state.loadingGettingPlayerStatus === 'pending')
          if (action.payload) {
      ...
      .addCase(getPlayersByTeamIdThunk.fulfilled, (state, action) => {
        if (state.loadingGettingTeamPlayersStatus === 'pending')
          if (action.payload) {
            state.loadingCreatingNewPlayerStatus = 'succeeded'
      ...
      .addCase(getPlayersByUserIdThunk.fulfilled, (state, action) => {
        if (state.loadingGettingTeamPlayersByUsersStatus === 'pending')
          if (action.payload) {
...
export const { clearPlayerState } = playersSlice.actions
