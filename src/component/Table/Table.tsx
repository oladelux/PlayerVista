      ...
      <table className='Table__container'>
          ...
          <tr className='Table__container-head-row'>
            {columns.map((column, index) => (
              <th
                ...
                className={`Table__container-head-row-cell ${index > 0 && index < columns.length - 2 ? 'hidden md:table-cell' : ''}`}
              >
                {column.title}
              </th>
            ))}
        ...
        <tbody className='Table__container-body'>
          {data.length ? (
            data.map((row, index) => (
              <tr key={index} className='Table__container-body-row'>
                {columns.map((column, xIndex) => (
                  ...
                  >
                    {column.render
                      ? column.render(row[column.key] as T)
            ...
            <tr>
              <td colSpan={columns.length} className='Table__container-body--no-data'>
