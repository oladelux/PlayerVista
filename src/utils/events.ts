...
export const filterEventsByMonth =
  (data: CalenderEvents[], eventType: EventType, targetMonth: number) => {
...
export const eventTrend = (
  ...
  lastMonthEvents:CalenderEvents[]): EventTrendResponse | null => {
  if (thisMonthEvents.length === 0 || lastMonthEvents.length === 0) {
    return null
  } else {
    const trendPercentage =
      ((thisMonthEvents.length - lastMonthEvents.length) / Math.abs(lastMonthEvents.length))
      * 100
    return {
      trendPercentage,
