...
export const PlayerSummaryStats: FC<PlayerSummaryStatsProps> = ({
  players,
}) => {
  ...
  const sortedTeamPlayers = teamPlayers.sort((a, b) => {
    return (
      predefinedOrder.indexOf(a.position) - predefinedOrder.indexOf(b.position)
    )
  })
  ...
  const [playerLocations, setPlayerLocations] =
    useState<LocationData[]>(currentLocations)
  const [heatSectors, setHeatSectors] =
    useState<HeatSectorsType[]>(currentHeatSectors)
