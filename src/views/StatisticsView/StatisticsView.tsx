...
export const StatisticsView: FC<StatisticsViewProps> = ({ events }) => {
  ...
  const teamEvents = events[teamId];

  function getTeamEvent(year: number) {
    const teamCurrentEvent = teamEvents.filter(
      (event) => new Date(event.startDate).getFullYear() === year
    );
    setSelectedEvent(teamCurrentEvent);
  }
